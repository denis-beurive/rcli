...
pub struct CommandLine<'a> {
    /// Optional parameters with an "explicit" value.
    /// Ex: "--input=<value>" or "-i=<value>".
    option: HashMap<Name, OptionalParameter>,
    /// Optional parameters with an "implicit" value (true/set or false/unset).
    /// Ex: "--verbose" or "-v".
...
impl<'a> CommandLine<'_> {
    ...
    pub fn add_option(&mut self, name: String, long: Option<String>, short: Option<char>, max_occurrence:Option<u8>) {
    ...
    pub fn add_flag(&mut self, name: String, long: Option<String>, short: Option<char>, max_occurrence:Option<u8>) {
        if self.option.contains_key(name.as_str()) {
    ...
    pub fn add_parameter(&mut self, name: String, max_occurrence: Option<u8>) {
    ...
    fn add_action(&mut self, name: String, command_line: Option<CommandLine>) -> CommandLine {
